...
class ODIR5KDataset(Dataset):
    ...
    def __init__(self, root_dir, csv_file, transform=None, mode='train', blur_threshold=100):
        ...
        Args:
            ...
            csv_file (str): Path to 'full_df.csv'.
            transform (callable, optional): Optional transform.
            mode (str): 'train' or 'val'.
            blur_threshold (float): Threshold for Laplacian variance.
        """
    ...
    def _calculate_blur(self, pil_img):
        """
        Calculate Laplacian variance using pure numpy and PIL.
        """
        # Standardize size for consistent blur thresholding
        img = pil_img.convert('L').resize((224, 224))
        img_array = np.array(img, dtype=np.float32)
        
        # Manual Laplacian convolution using numpy
        # Kernel: [[0, 1, 0], [1, -4, 1], [0, 1, 0]]
        # We can implement this by shifting the array
